...
public class SeasonDAOImpl {
	...
	public void generateSeasons(int showId, int numOfSeasons, List<String> imagePaths) throws SQLException {
		String insertSql = "INSERT OR IGNORE INTO seasons(show_id, title, image_path) VALUES(?, ?, ?)";
		...
		try(PreparedStatement insertStmt = conn.prepareStatement(insertSql);
			...
			for(int i = 1; i <= numOfSeasons; i++) {
				...
				insertStmt.setString(2, "Season " + i);
	...
	public List<Season> getSeasonsByShowId(int showId) throws SQLException {
		...
		String sql = "SELECT s.id, s.show_id, s.title, s.image_path, "
				   ...
				   + "WHERE s.show_id = ? "
				   + "GROUP BY s.id, s.show_id, s.title, s.image_path "
				   + "ORDER BY s.id";
