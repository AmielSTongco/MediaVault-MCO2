...
public class MediaPlaylistsController implements Initializable {
	...
	private void setupTableCells() {
		...
		creatorColumn.setCellFactory(column -> new TableCell<Media, String>() {
			private final Label label = new Label();
			private final StackPane wrapper = new StackPane(label);
		...
		yearColumn.setCellFactory(column -> new TableCell<Media, String>() {
			private final Label label = new Label();
			private final StackPane wrapper = new StackPane(label);

			{
				label.getStyleClass().add("media-cell");
				wrapper.setAlignment(Pos.CENTER_LEFT);
				wrapper.setPadding(new Insets(0, 0, 0, 4));
			}
		...
		statusColumn.setCellFactory(column -> new TableCell<Media, String>() {
			private final Label label = new Label();
			private final StackPane wrapper = new StackPane(label);
		...
		ratingColumn.setCellValueFactory(cell -> new ReadOnlyStringWrapper(
				cell.getValue().getUserRatingString()
			));
		
		ratingColumn.setCellFactory(column -> new TableCell<Media, String>() {
			private final Label label = new Label();
			private final StackPane wrapper = new StackPane(label);
		...
		reviewColumn.setCellValueFactory(cell -> new ReadOnlyStringWrapper(
				cell.getValue().getReviewedStatus()
			));
		
		reviewColumn.setCellFactory(column -> new TableCell<Media, String>() {
			private final Label label = new Label();
			private final StackPane wrapper = new StackPane(label);
		...
		infoColumn.setCellFactory(column -> new TableCell<Media, String>() {
			private final Label label = new Label();
			private final StackPane wrapper = new StackPane(label);
